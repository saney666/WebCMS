		...
		function () {
			...
			if (!jQuery('#ptTimeSelectCntr').length) {
				jQuery("body").append(
					...
					+	'				<span id="ptTimeSelectUserSelHr">0</span> : '
					...
					+	'					<div class="ptTimeSelectLeftPane" style="width: 50%; text-align: center; float: left;"></div>'
					+	'					<div class="ptTimeSelectRightPane" style="width: 50%; text-align: center; float: left;"></div>'
					+	'				</div>'
					+	'				<div>'
					...
					+	'										style="display: block; width: 43%; float: left;">AM</a>'
					+	'								<a class="ptTimeSelectHr ui-state-default" href="javascript: void(0);" '
					+	'										style="display: block; width: 43%; float: left;">PM</a>'
...
jQuery.ptTimeSelect.openCntr = function (ele) {
    ...
    if (cntr.find(".ptTimeSelectLeftPane .ptTimeSelectHrCntr a").length == 0)
        for (var key in opt.hoursList)
            cntr.find(".ptTimeSelectLeftPane .ptTimeSelectHrCntr br").before('<a class="ptTimeSelectHr ui-state-default" href="javascript: void(0);">' + opt.hoursList[key] + '</a>');
    cntr.find(".ptTimeSelectTimeLabelsCntr .ptTimeSelectRightPane").empty().append(opt.minutesLabel);
    if (cntr.find(".ptTimeSelectRightPane .ptTimeSelectMinCntr a").length == 0)
        for (var key in opt.minutesList)
            cntr.find(".ptTimeSelectRightPane .ptTimeSelectMinCntr br").before('<a class="ptTimeSelectMin ui-state-default" href="javascript: void(0);">' + opt.minutesList[key] + '</a>');
